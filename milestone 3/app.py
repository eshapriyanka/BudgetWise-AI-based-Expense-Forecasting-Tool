...
def process_uploaded_data(df_raw, username):
    ...
    # Preserve original category BEFORE lowercasing
...
# Main Application
        ...
        if uploaded_file is not None:
            if uploaded_file != st.session_state.current_uploaded_file or not st.session_state.uploaded_file_processed:
                ...
                try:
                    ...
                    st.error(f"Error during file upload/processing: {e}"); st.exception(e)
                    st.session_state.current_uploaded_file = None; st.session_state.uploaded_file_processed = False; st.session_state.transactions = []
        ...
        if not df.empty:
            try:
                ...
                if null_dates_final > 0:
                     if 'invalid_date_count' not in st.session_state or st.session_state.invalid_date_count == 0: 
                         st.warning(f"Note: {null_dates_final} invalid dates excluded from analysis.")
                     df.dropna(subset=['Date'], inplace=True)
                if not df.empty:
                     expenses_df = df[df['Type'] == 'Expense'].copy()
            except Exception as e:
                st.error(f"Date error during analysis prep: {e}"); st.stop()
                                ...
                                if not new_rows_in_editor.empty:
                                     ...
                                     for row in new_rows_dict:
                                         ...
                                         if pd.notna(row.get('Date')):
                                              row['Category'] = categorize_transaction_nltk(row.get('Description', ''), row.get('Type', 'Expense'))
    ...
    if st.session_state.transactions and not df.empty: # Check if df was created
        with st.spinner("Analyzing data & building charts..."):
            ...
            # Charts
            if not expenses_df.empty: # Check if expenses_df was created
                ...
                try:
                    if pd.api.types.is_datetime64_any_dtype(expenses_df['Date']):
                        ...
                        if years:
                            year=st.selectbox("Select Year:", years); yearly=expenses_df[expenses_df['Year']==year]
                            if not yearly.empty:
        ...
        st.header("Future Expense Forecast")

        if not expenses_df.empty and len(expenses_df.index) > 30:
            try:
                ...
                if len(daily_expenses.index) < 30:
                     st.info("Not enough historical data points (need >30 days) for forecasting.")
                else:
                    # Historical Data View Selector 
                    col_opt, col_days = st.columns([1, 1])
                    with col_opt:
                        history_view = st.selectbox("Select Historical Data View:", 
                                                    ['Weekly Average', 'Daily Total (All Data)', 'Monthly Average', 'Yearly Average'])
                    with col_days:
                    ...
                    if st.button("Generate Forecast", use_container_width=True):
                        with st.spinner(f"Training model and forecasting {forecast_days} days..."):
                            ...
                            # Dynamic Data Prep for Altair Chart based on selection
                            if history_view == 'Daily Total (All Data)':
                                actuals_to_plot = daily_expenses.rename(columns={'y': 'Actual'})
                                tooltip_format = '%Y-%m-%d'
                                legend_label = "Actual Daily Total"
                            elif history_view == 'Weekly Average (Recommended)':
                                actuals_to_plot = daily_expenses.set_index('ds').resample('W')['y'].mean().reset_index().rename(columns={'y': 'Actual'})
                                tooltip_format = 'Week of %Y-%m-%d'
                                legend_label = "Actual Weekly Average"
                            elif history_view == 'Monthly Average':
                                actuals_to_plot = daily_expenses.set_index('ds').resample('ME')['y'].mean().reset_index().rename(columns={'y': 'Actual'})
                                tooltip_format = 'Month %Y-%m'
                                legend_label = "Actual Monthly Average"
                            else: # Yearly
                                actuals_to_plot = daily_expenses.set_index('ds').resample('YE')['y'].mean().reset_index().rename(columns={'y': 'Actual'})
                                tooltip_format = 'Year %Y'
                                legend_label = "Actual Yearly Average"

                            forecast_to_plot = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']]
                            
                            base = alt.Chart(forecast_to_plot).mark_line().encode(
                                x=alt.X('ds:T', title='Date'),
                                y=alt.Y('yhat:Q', title='Expense Amount ($)', scale=alt.Scale(zero=False))
                            ).interactive()
                            
                            forecast_line = base.mark_line(color='#0d47a1').encode(
                                tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('yhat:Q', title='Predicted', format='$,.2f')]
                            )
                            confidence_band = base.mark_area(opacity=0.3, color='#42a5f5').encode(y='yhat_lower:Q', y2='yhat_upper:Q')
                            
                            actuals_points = alt.Chart(actuals_to_plot).mark_circle(color='black', size=15, opacity=0.7).encode(
                                x=alt.X('ds:T', title='Date'),
                                y=alt.Y('Actual:Q', title='Expense Amount ($)'),
                                tooltip=[alt.Tooltip('ds:T', title='Date', format=tooltip_format.replace('Week of ', '').replace('Month ', '').replace('Year ', '')), alt.Tooltip('Actual:Q', title='Actual', format='$,.2f')]
                            )
                            
                            final_chart = confidence_band + forecast_line + actuals_points
                            st.markdown(f"**Chart Legend:**\n* **Black Dots:** {legend_label}\n* **Blue Line:** Predicted future spending (`yhat`)\n* **Light Blue Area:** 80% Confidence interval")
                            st.altair_chart(final_chart, use_container_width=True)
                            
                            # Components Chart (Filtered)
                            st.subheader("Forecast Trends & Seasonality (Future Period)")
                            last_historical_date = daily_expenses['ds'].max()
                            components_df = forecast[forecast['ds'] > last_historical_date][['ds', 'trend', 'weekly', 'yearly']].copy()
                            components_df_melted = components_df.melt('ds', var_name='Component', value_name='Value')
                            component_chart = alt.Chart(components_df_melted).mark_line().encode(
                                x=alt.X('ds:T', title='Date'), y=alt.Y('Value:Q', title='Component Value ($)'),
                                color=alt.Color('Component:N', title='Trend/Seasonality'),
                                row=alt.Row('Component:N', title="", header=alt.Header(titleOrient="bottom", labelOrient="bottom")),
                                tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('Value:Q', title='Value ($)', format=',.2f')]
                            ).properties(title="Forecast Components").resolve_scale(y='independent').interactive()
                            st.altair_chart(component_chart, use_container_width=True)

                            # Data Table 
                            st.write(f"Forecasted Values for the Next {forecast_days} Days:")
                            future_forecast = forecast[forecast['ds'] > daily_expenses['ds'].max()]
                            future_forecast_display = future_forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].set_index('ds')
                            for col in ['yhat', 'yhat_lower', 'yhat_upper']: future_forecast_display[col] = future_forecast_display[col].map('${:,.2f}'.format)
                            st.dataframe(future_forecast_display)

            except Exception as e: st.error(f"Forecasting error: {e}"); st.exception(e)
        else:
             st.info("Add more expense data (at least 30 days) to enable forecasting.")
        # END 
